...
const SecurePortal: React.FC<Props> = ({ navigate, onAuthenticated }) => {
  ...
  const handleEntry = (role: UserRole) => {
    ...
    setTimeout(() => {
      setIsValidating(false);
      // For demo/simulated entry, we check if it matches a generic master key if not already caught
      if (portKey === 'URKIO-CLINICAL-2025') {
        setStatus('GRANTED');
  ...
  return (
    <div className="relative h-screen w-full flex flex-col bg-[#050b1a] text-white max-w-md mx-auto overflow-hidden font-sans">
      ...
      <main className="relative z-10 flex-1 px-8 flex flex-col items-center justify-center">
        ...
        <div className={`w-full p-8 rounded-[3rem] border-2 transition-all duration-700 ${status === 'DENIED' ? 'border-pink-600 bg-pink-600/5' :
            status === 'GRANTED' ? 'border-emerald-500 bg-emerald-500/5' :
              'border-primary/30 bg-white/5 shadow-2xl shadow-primary/10'
          }`}>
          <div className="flex flex-col items-center text-center gap-6">
            <div className={`size-24 rounded-[2rem] border-2 flex items-center justify-center relative overflow-hidden transition-all duration-500 ${status === 'SCANNING' ? 'border-accent-cyan' : 'border-white/10'
              }`}>
              {status === 'SCANNING' && (
              ...
              <span className={`material-symbols-outlined text-5xl ${status === 'DENIED' ? 'text-pink-600 animate-shake' :
            ...
            <div className="w-full space-y-4">
              <div className="space-y-1">
                ...
                />
                {status === 'DENIED' && (
                  <p className="text-[9px] font-black text-pink-600 uppercase tracking-widest animate-pulse mt-2">
        ...
        <button
          ...
          className="mt-12 group flex items-center gap-2 text-[9px] font-black uppercase tracking-[0.4em] text-slate-600 hover:text-white transition-all"
        >
