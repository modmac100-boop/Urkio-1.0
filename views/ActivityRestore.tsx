...
const ActivityRestore: React.FC<{ navigate: (s: AppScreen) => void, language: 'en' | 'ar' }> = ({ navigate, language }) => {
        ...
        <section className="space-y-8 relative">
           ...
           <div className="space-y-6">
              {activities.length > 0 ? activities.map((act, i) => (
                ...
                >
                   {/* Timeline Node */}
                   <div className={`absolute left-0 top-0 size-16 rounded-[1.8rem] crystal-glass border-2 flex items-center justify-center z-10 transition-all duration-500 group-hover:scale-110 ${
                     act.status === 'RESTORED' ? 'border-emerald-500 bg-emerald-500/10' : 
                     act.status === 'RESTORING' ? 'border-primary bg-primary/10 animate-pulse' : 
                     'border-white/10 group-hover:border-primary/40'
                   }`}>
                      <span className={`material-symbols-outlined text-2xl ${
                        act.status === 'RESTORED' ? 'text-emerald-500' : 
                   ...
                   <div className={`p-7 rounded-[2.5rem] border transition-all duration-700 ${
                     act.status === 'RESTORED' ? 'bg-emerald-500/5 border-emerald-500/20' : 
