...
const NotificationsView: React.FC<{ navigate: (s: AppScreen, e?: Expert) => void, language: 'en' | 'ar' | 'fr' }> = ({ navigate, language }) => {
  ...
  const t = language === 'ar' ? {
    ...
    sub: 'Le pouls de votre parcours',
      ...
      <main className="p-4 space-y-3">
        {notifs.map((n) => (
          <div
            ...
            className={`relative p-5 rounded-[2rem] border transition-all cursor-pointer group active:scale-[0.98] ${n.isRead ? 'bg-white/40 dark:bg-white/5 border-transparent opacity-80' : 'bg-white dark:bg-slate-900 border-gray-100 dark:border-primary/20 shadow-lg'
              }`}
          >
            {!n.isRead && (
            ...
            <div className="flex gap-4">
              <div className={`size-12 rounded-[1.2rem] shrink-0 flex items-center justify-center border transition-all ${n.type === 'EXPERT_NOTE' ? 'bg-primary/10 text-primary border-primary/20' :
