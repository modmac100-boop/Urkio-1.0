...
const FounderAdminPanel: React.FC<{ navigate: (s: AppScreen) => void }> = ({ navigate }) => {
         ...
         <div className="flex p-1.5 bg-white/5 rounded-3xl border border-white/5 shadow-inner">
            {(['TELEMETRY', 'FINANCE', 'OVERRIDE'] as const).map(tab => (
              <button 
                ...
                className={`flex-1 py-3.5 rounded-2xl text-[9px] font-black uppercase tracking-widest transition-all ${
                   activeTab === tab ? 'bg-cyan-500 text-black shadow-[0_0_20px_rgba(0,242,255,0.5)]' : 'text-slate-500 hover:text-slate-300'
                }`}
              >
                {tab}
              </button>
                        ...
                        <div className="flex justify-between items-start mb-4">
                           <div className="flex items-center gap-4">
                              <div className={`size-10 rounded-xl flex items-center justify-center ${
                                event.status === 'CRITICAL' ? 'bg-red-500 text-white shadow-lg shadow-red-500/30' :
             ...
             <section className="bg-red-500/5 border border-red-500/20 rounded-[3rem] p-8 text-center space-y-8">
                ...
                <div className="grid grid-cols-1 gap-3">
                   <button 
                     ...
                     className={`w-full h-16 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all ${
                       isEmergencyActive ? 'bg-white text-red-600 shadow-2xl' : 'bg-red-600 text-white shadow-xl shadow-red-600/20'
                     }`}
                   >
