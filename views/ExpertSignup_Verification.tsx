...
const ExpertSignup_Verification: React.FC<Props> = ({ navigate, expertDraft, setExpertDraft }) => {
  const [formData, setFormData] = useState({
    fullName: expertDraft.fullName || 'Dr. Aris Varma',
    email: expertDraft.email || 'aris.varma@clinical.com',
    phone: expertDraft.phone || '+1 (555) 012-3456',
    professionalId: expertDraft.professionalId || 'MED-99281-X',
  });

  const [isUploadingCert, setIsUploadingCert] = useState(false);
  const [isUploadingCv, setIsUploadingCv] = useState(false);
  const [certFile, setCertFile] = useState<File | null>(expertDraft.documents?.license || null);
  const [cvFile, setCvFile] = useState<File | null>(expertDraft.documents?.cv || null);
  const [letter, setLetter] = useState(expertDraft.letter || '');
  ...
  const handleSubmit = () => {
    ...
    // Save all verification data to the persistent draft
    setExpertDraft({
      ...expertDraft,
          ...
          <section className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div
                ...
                className={`h-40 rounded-[2.5rem] border-2 border-dashed flex flex-col items-center justify-center p-4 text-center gap-2 transition-all relative overflow-hidden group cursor-pointer ${certFile ? 'bg-emerald-500/10 border-emerald-500' : 'bg-white/5 border-white/10 hover:border-primary/50'
                  }`}
              >
                {isUploadingCert ? (
              ...
              <div
                ...
                className={`h-40 rounded-[2.5rem] border-2 border-dashed flex flex-col items-center justify-center p-4 text-center gap-2 transition-all relative overflow-hidden group cursor-pointer ${cvFile ? 'bg-emerald-500/10 border-emerald-500' : 'bg-white/5 border-white/10 hover:border-primary/50'
                  }`}
              >
                {isUploadingCv ? (
                  <div className="flex flex-col items-center gap-2">
