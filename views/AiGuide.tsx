...
const ThinkingSkeleton = ({ language }: { language: 'en' | 'ar' }) => (
      ...
      <div className="relative z-10 space-y-4">
        ...
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-[8px] font-black uppercase tracking-[0.3em] text-primary">
            {language === 'ar' ? 'جاري تحليل البيانات...' : 'Neural Core Processing...'}
          </span>
        </div>
      ...
      <main ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-6 pb-24">
        ...
        {messages.map((msg) => (
          <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[85%] rounded-[1.8rem] px-5 py-4 shadow-sm ${
              msg.role === 'user' 
