...
const BoardAdminPanel: React.FC<{ navigate: (s: AppScreen) => void, handleLogout: () => void }> = ({ navigate, handleLogout }) => {
         ...
         <header className="sticky top-0 z-50 bg-[#020617]/90 backdrop-blur-xl p-6 border-b border-white/5 flex items-center justify-between shadow-2xl">
            <div className="flex items-center gap-4">
               <button
                  ...
                  className="size-11 flex items-center justify-center rounded-2xl bg-white/5 border border-white/10 text-slate-500 hover:text-white transition-all mr-2"
               >
            ...
            <div className="flex p-1 bg-white/5 rounded-2xl border border-white/5 shadow-inner">
               {(['TELEMETRY', 'DISCOVER', 'SECURITY'] as const).map(tab => (
                  <button
                     ...
                     className={`flex-1 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${activeTab === tab ? 'bg-primary text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'
                        }`}
                  >
                     {tab}
                  </button>
               ...
               <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
                  ...
                  <div className="space-y-4">
                     {logs.map((log, i) => (
                        ...
                        >
                           <div className={`absolute left-0 top-0 bottom-0 w-1 ${log.status === 'FLAGGED' ? 'bg-red-500' :
